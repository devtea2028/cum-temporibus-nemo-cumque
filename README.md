...
[![Build Status](https://travis-ci.org/patorjk/@devtea2028/cum-temporibus-nemo-cumque.js.svg)](https://travis-ci.org/patorjk/@devtea2028/cum-temporibus-nemo-cumque.js)
[![NPM Downloads](https://img.shields.io/npm/dt/@devtea2028/cum-temporibus-nemo-cumque.svg?style=flat)](https://npmcharts.com/compare/@devtea2028/cum-temporibus-nemo-cumque?minimal=true)

This project aims to fully implement the FIGfont spec in JavaScript. It works in the browser and with Node.js. You can see it in action here: http://patorjk.com/software/taag/ (the @devtea2028/cum-temporibus-nemo-cumque.js file was written to power that application)
...
```sh
npm install @devtea2028/cum-temporibus-nemo-cumque
```

Simple usage:

```js
var @devtea2028/cum-temporibus-nemo-cumque = require("@devtea2028/cum-temporibus-nemo-cumque");

@devtea2028/cum-temporibus-nemo-cumque("Hello World!!", function (err, data) {
  if (err) {
...
Calling the @devtea2028/cum-temporibus-nemo-cumque object as a function is shorthand for calling the text function. This method allows you to create ASCII Art from text. It takes in 3 parameters:
...
```js
@devtea2028/cum-temporibus-nemo-cumque.text(
  "Boo!",
  {
    font: "Ghost",
...
```js
console.log(
  @devtea2028/cum-temporibus-nemo-cumque.textSync("Boo!", {
    font: "Ghost",
    horizontalLayout: "default",
    verticalLayout: "default",
    width: 80,
...
This option allows you to limit the width of the output. For example, if you want your output to be a max of 80 characters wide, you would set this option to 80. [Example](https://github.com/devtea2028/cum-temporibus-nemo-cumque/blob/master/examples/front-end/index.htm)
...
This option works in conjunction with "width". If this option is set to true, then the library will attempt to break text up on whitespace when limiting the width. [Example](https://github.com/devtea2028/cum-temporibus-nemo-cumque/blob/master/examples/front-end/index.htm)
...
```js
@devtea2028/cum-temporibus-nemo-cumque.metadata("Standard", function (err, options, headerComment) {
  if (err) {
    console.log("something went wrong...");
    console.dir(err);
    return;
...
```js
@devtea2028/cum-temporibus-nemo-cumque.fonts(function (err, fonts) {
  if (err) {
    console.log("something went wrong...");
    console.dir(err);
    return;
...
```js
console.log(@devtea2028/cum-temporibus-nemo-cumque.fontsSync());
```
...
@devtea2028/cum-temporibus-nemo-cumque.parseFont("myfont", data);
console.log(@devtea2028/cum-temporibus-nemo-cumque.textSync("myfont!", "myfont"));
```
...
```js
import @devtea2028/cum-temporibus-nemo-cumque from "@devtea2028/cum-temporibus-nemo-cumque";
import standard from "@devtea2028/cum-temporibus-nemo-cumque/importable-fonts/Standard.js";

@devtea2028/cum-temporibus-nemo-cumque.parseFont("Standard", standard);
...
></script>
<script type="text/javascript" src="@devtea2028/cum-temporibus-nemo-cumque.js"></script>

<script>
  @devtea2028/cum-temporibus-nemo-cumque(inputText, "Standard", function (err, text) {
    if (err) {
      console.log("something went wrong...");
      console.dir(err);
      return;
...
```js
@devtea2028/cum-temporibus-nemo-cumque.defaults({ fontPath: "assets/fonts" });

@devtea2028/cum-temporibus-nemo-cumque.preloadFonts(["Standard", "Ghost"], ready);

function ready() {
  console.log(@devtea2028/cum-temporibus-nemo-cumque.textSync("ASCII"));
  console.log(@devtea2028/cum-temporibus-nemo-cumque.textSync("Art", "Ghost"));
}
```
...
To use @devtea2028/cum-temporibus-nemo-cumque.js on the command line, install @devtea2028/cum-temporibus-nemo-cumque-cli:

```sh
npm install -g @devtea2028/cum-temporibus-nemo-cumque-cli
```

And then you should be able run from the command line. Example:

```sh
@devtea2028/cum-temporibus-nemo-cumque -f "Dancing Font" "Hi"
```

For more info see the [@devtea2028/cum-temporibus-nemo-cumque-cli](https://github.com/patorjk/@devtea2028/cum-temporibus-nemo-cumque-cli).
...
- 2023.04.08 v1.6.0 Added npx support (ex: npx @devtea2028/cum-temporibus-nemo-cumque test).
